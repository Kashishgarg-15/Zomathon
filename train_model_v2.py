# -*- coding: utf-8 -*-
"""CSAO Rail v2 - Temporal split, NDCG@10 Optuna, 40 trials, LightGBM + XGBoost"""
...
# ----------------------------------------------------------------
# HELPER: NDCG@K for a set of grouped predictions
# ----------------------------------------------------------------
def compute_ndcg(order_ids, y_true, y_pred, k=10):
    """Compute mean NDCG@k grouped by order_id."""
...
# ================================================================
# STEP 2: FEATURE ENGINEERING (same 9 features)
# ================================================================
...
# ================================================================
# STEP 5: TEMPORAL TRAIN/VAL/TEST SPLIT  *** FIX 1 ***
# ================================================================
log("\nSTEP 4: TEMPORAL Train/Val/Test split...")
df['_base'] = df['order_id'].str.replace(r'^(syn_|soft_)', '', regex=True)

# Extract numeric part from "ord_XXXXX" for temporal ordering
...
base_ids = df['_base'].unique()
# Sort by the order number -> temporal ordering
base_ids_sorted = sorted(base_ids, key=extract_order_num)
...
log(f"  No base_order leakage: {len(train_ids & val_ids) == 0 and len(val_ids & test_ids) == 0}")

# ================================================================
# STEP 6: LIGHTGBM OPTUNA (40 trials, NDCG@10)  *** FIX 2+3 ***
# ================================================================
log("\n" + "=" * 60)
log("STEP 5: LightGBM tuning (20 trials, optimizing NDCG@10)...")
t1 = time.time()
...
# ================================================================
# STEP 8: XGBOOST OPTUNA (40 trials, NDCG@10)
# ================================================================
log("\n" + "=" * 60)
log("STEP 7: XGBoost tuning (15 trials, optimizing NDCG@10)...")
t2 = time.time()

# Pre-create DMatrix objects outside the objective to avoid re-creation overhead
...
log(f"  XGB Optuna done")
log(f"  Params: {study2.best_params}")

# ================================================================
# STEP 9: FINAL XGBOOST
# ================================================================
...
# ================================================================
# STEP 10: ENSEMBLE (optimize weight on val NDCG@10)
# ================================================================
log("\n" + "=" * 60)
log("STEP 9: Ensemble (optimize weight by val NDCG@10)...")
...
# ================================================================
# STEP 11: FULL EVALUATION ON TEST SET
# ================================================================
log("\n" + "=" * 60)
...
# Also compute ranking for individual models
log("\n  Ranking metrics (LightGBM):")
er_lgb = ranking_metrics(test_oids, yb['test'].values, lp_t)
for k, v in sorted(er_lgb.items()):
    log(f"    {k:12s}: {v:.4f}")

log("\n  Ranking metrics (XGBoost):")
er_xgb = ranking_metrics(test_oids, yb['test'].values, xp_t)
for k, v in sorted(er_xgb.items()):
    log(f"    {k:12s}: {v:.4f}")
...
res = {
    'version': 'v2',
    'fixes': ['temporal_split', 'ndcg10_optuna', '40_trials'],
    'data': {
        'rows': int(df.shape[0]), 'feats': len(FEATS),
        'train': len(X['train']), 'val': len(X['val']), 'test': len(X['test']),
        'temporal_order': True,
        'train_order_range': f'..{train_max}',
        'val_order_range': f'{val_min}..{val_max}',
        'test_order_range': f'{test_min}..',
    },
    'lgb': {
        'iter': lgb_final.best_iteration, 'params': bp1, 'metrics': lm,
        'ranking': er_lgb,
        'optuna': {'trials': 40, 'objective': 'ndcg@10', 'best_val': study1.best_value}
    },
    'xgb': {
        'params': bp2, 'metrics': xm,
        'ranking': er_xgb,
        'optuna': {'trials': 40, 'objective': 'ndcg@10', 'best_val': study2.best_value}
    },
    'ensemble': {
...
# Save val predictions too (for later DCN-v2 stacking)
pd.DataFrame({
    'order_id': val_oids,
